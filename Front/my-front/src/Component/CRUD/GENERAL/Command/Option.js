...
export default function Option({idCommand}) {
  ...
  useEffect(() => {
    if (idCommand) {
        axios.get(`${apiUrl}/optionByCmd/${idCommand}`)
          .then((response) => {
  ...
  const handleDeleteOption=(idOption) =>{
    axios.delete(`${apiUrl}/option/${idOption}`)
    .then(()=>{
      ...
      {idCommand ? (
        ...
        <div>
          ...
          <table className="machine-table">
            ...
            <tbody>
              {data
                .filter((item) => item.optionSyntax.toLowerCase().includes(searchQuery.toLowerCase()))
                .map((rowData, index) => (
